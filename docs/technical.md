...
| 10 | **Localizer uses `amcl_pose`, not depth** | Suspect is reported at the robot's own pose, not the person's true position | Deliberate: the X5 CPU already sits ~90 % under the full stack + VLM, so the heavier `pointcloud` centroid+TF path is avoided. Drive up to whoever you're checking. Switch to `location_source:=pointcloud` only if you free up headroom. |
